...
$api->header(array( 'page-title' => 'Акционные предложения' ));
                ...
                foreach ($subList as $subO) {
                   ...
                   if($subO['class_id']==$offersId)
                    $out = array_merge($out,getLiLeftMenu($o,$subO));
                     ...
                     foreach ($subO as $sub2) {

                   if($sub2['class_id']==$offersId)
                    $out = array_merge($out,getLiLeftMenu($o,$subO));
            }
        ...
        return '
            <ul class="sidebar_menu accord_menu">
                <li '.($_SERVER['REQUEST_URI'] == '/'.$api->lang.'/'.$api->section->sectionName.'/service_offers/'?'class="active"':'').'>
                    <a href="/'.$api->lang.'/'.$api->section->sectionName.'/service_offers/">Акционные предложения</a>
                </li>
                '.join("\n", $out).'
            </ul>';
    }
