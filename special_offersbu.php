...
# ---------- СПИСОК СПЕЦ ПРЕДЛОЖЕНИЙ --------------------
    ...
    if ($offersSectionsList = $api->objects->getFullObjectsListByCLass($api->section->sectionId, $offersSectionId)) {
        ...
        foreach ($offersSectionsList as $oSo) {
            ...
            if ($offersList = $api->objects->getFullObjectsListByCLass($oSo['id'], $offersId)) {
                foreach ($offersList as $oo) {
                    ...
                    $offersHref = 'href="/'.$api->lang.'/'.$api->section->sectionName.'/special_offers/section/'.$oSo['id'].'/'.$oo['id'].'/"';
