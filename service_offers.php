...
$api->header(array( 'page-title' => 'Специальные предложения' ));
...
function getLeftMenu() {
    ...
    global $api, $offersSectionId, $offersId, $root_id;

    $out = array();
    if ($list = $api->objects->getFullObjectsListByCLass($root_id, $offersSectionId)) {
        foreach ($list as $o) {
            if ($subList = $api->objects->getFullObjectsListByCLass($o['id'], $offersId)) {
                foreach ($subList as $subO) {
                    ...
                    $out[] = '<li '.$class.'><a href="/'.$api->lang.'/'.$api->section->sectionName.'/service_offers/section/'.$o['id'].'/'.$subO['id'].'/">'.$subO['Название'].'</a></li>';
                }
            }
        }
        return '
            <ul class="sidebar_menu accord_menu">
                <li '.($_SERVER['REQUEST_URI'] == '/'.$api->lang.'/'.$api->section->sectionName.'/service_offers/'?'class="active"':'').'>
                    <a href="/'.$api->lang.'/'.$api->section->sectionName.'/service_offers/">Специальные предложения</a>
                </li>
                '.join("\n", $out).'
            </ul>';
    }
