...
# ----------------------------------------------------- #
# --------- АНАЛОГ РАЗДЕЛА АВТОКРЕДИТОВАНИЕ ----------- #
# --------- ПОЭТОМУ МОГУТ НАБЛЮДАТЬСЯ ПЕРЕСИЧЕНИЕ ----- #
# ---------     В ИМЕНАХ ПЕРЕМЕННЫХ/МАССИВОВ      ----- #
# ----------------------------------------------------- #
...
$sectionObj['id'] = $root;
$sectionObj['Название'] = 'Автострахование';
...
if (@$_GET['insuranceId'] && is_numeric($_GET['insuranceId'])) {

    $crediProgrObj = $api->objects->getFullObject($_GET['insuranceId']);

    $title = $crediProgrObj['Название'];
            ...
            <div class="left_column">
                <div class="wrap">
                    <ul class="sidebar_menu accord_menu">
                        <li '.($_SERVER['REQUEST_URI'] ==  '/'.$api->lang.'/'.$api->section->sectionName.'/auto_insurance/'?'class="active"':'').'>
                            <a href="/'.$api->lang.'/'.$api->section->sectionName.'/auto_insurance/">'.$sectionObj['Название'].'</a>
                        </li>
                        '.$api->getLeftMenu($sectionObj['id'], '/'.$api->lang.'/'.$api->section->sectionName.'/auto_insurance/', @$_GET['insuranceId']).'
                    </ul>
                </div>
            </div>
            <div class="right_column">
                <div class="cont">
                    ...
                    <div class="simple_text">
                        '.$crediProgrObj['Текст'].'
                        <!--smart:{ id:'.$crediProgrObj['id'].', title:"текста", actions:["edit"] }-->
                    </div>
