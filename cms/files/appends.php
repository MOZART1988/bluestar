...
var $dbpass;
	...
	function __construct(){
        ...
        $this->dbpass = '';
//        $this->dbhost = 'localhost';
//        $this->dbname = 'mersedes_1';
//        $this->dblogin = 'mersedes_1';
//        $this->dbpass = 'G77NAAo+[SZP';
		$this->db = new mysql($this->dbhost, $this->dbname, $this->dblogin, $this->dbpass);
	}
	...
	function areaJS( $text_or_array ){
		...
		return '<script type="text/javascript" charset="utf-8">'.$text_or_array.'</script>'."\n";
	}
	...
	function pages($total_count, $on_one_page, $view_page_links_per_side=5, $page_attributes=array(), $url = false, $lang = 'ru'){
		...
		if($current_page>1){ 
			if($start>1){ 
				if(!$url){ 
					// $html[]='<a class="backpage" href="?pg=1'.($attrs?'&'.$attrs:'').'">'.$vars[$lang]['в начало'].'</a>';
				}
				// else $html[]='<a class="backpage" href="'.str_replace("#pg#", 1, $url).'">'.$vars[$lang]['в начало'].'</a>';
			}
			if(!$url){ 
				$html[]='<a class="backpage" href="?pg='.($current_page-1).($attrs?'&'.$attrs:'').'"><span>Назад</span></a>';
			}
			else $html[]='<a class="backpage" href="'.str_replace("#pg#", ($current_page-1), $url).'"><span>Назад</span></a>';
		}
		
		for($i = intval($start); $i<=intval($end); $i++){
			if($i==$current_page) $html[]='<a class="num_str active" href="javascript:void(0)"><span>'.$i.'</span></a>';
			else{ 
				if(!$url){
					$html[]='<a class="num_str" href="?pg='.($i).($attrs?'&'.$attrs:'').'"><span>'.$i.'</span></a>';
				}else $html[]='<a class="num_str" href="'.str_replace("#pg#", $i, $url).'"><span>'.$i.'</span></a>';
			}
		}
		
		if($current_page!=$count_pages){ 
			if(!$url){ 
				$html[]='<a class="nextpage" href="?pg='.($current_page+1).($attrs?'&'.$attrs:'').'"><span>Вперед</span></a>';
			}
			else $html[]='<a class="nextpage" href="'.str_replace("#pg#", ($current_page+1), $url).'"><span>Вперед</span></a>';
			if($end!=$count_pages){ 
				if(!$url){
		...
		$out_html = '<div class="page_nav">'.join("\n", $html).'</div>';
